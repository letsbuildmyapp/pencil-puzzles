...
// Simplified tile sheet for Territory mode.
// Local drawing state only — the tile is claimed via the parent's onClaim callback
// once the drawing matches the reference.
export default function TerritoryTileSheet({ sel, puzzle, onClose, onClaim, locked }) {
  const [userTile, setUserTile] = useState(() => Array.from({ length: 5 }, () => Array(5).fill(0)));
  ...
  // When the tile is claimed externally (opponent beat me), the sheet plays its close
  // animation via this derived flag and the parent unmounts us after the timer below.
  const isClosing = closing || locked;

  // If the tile becomes claimed while I'm drawing, trigger onClose after the animation.
  useEffect(() => {
    if (!locked) return;
    ...
    <>
      ...
      >
        ...
        <div style={{ display: "flex", gap: 8, padding: "10px 20px 20px", borderTop: "1px solid rgba(255,255,255,0.07)", flexShrink: 0 }}>
          ...
          >
            {correctFlash ? "⚔ CLAIMED!" : showFeedback && !isTileDone ? "✗ NOT QUITE" : "⚔ CLAIM"}
          </button>
        </div>
      </div>
    </>
